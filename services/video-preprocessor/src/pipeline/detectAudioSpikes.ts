...
export type AudioSpikeSignal = { timestampSeconds: number; score: number };

/** Simple loudness proxy: sample audio in windows and return timestamps with normalized score. */
export async function detectAudioSpikes(
  filePath: string,
  _threshold: number
): Promise<AudioSpikeSignal[]> {
  ...
  try {
    ...
    const signals: AudioSpikeSignal[] = [];
    ...
    if (audioPath) unlinkSafe(audioPath);
  }
}
