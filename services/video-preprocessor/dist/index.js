...
import { mergeCandidateMoments } from "./pipeline/mergeCandidateMoments.js";
...
app.post("/candidates", async (req, res) => {
    const { videoUrl, proxyUrl, config: configOverrides } = req.body;
    ...
    try {
        ...
        const candidates = mergeCandidateMoments({ scene, motion, audio, speech }, durationSeconds, config);
        ...
        cleanupClips();
        const responseCandidates = candidates.map((c, i) => ({
            ...c,
            clipUrl: clipUrls[i],
        }));
        res.json({
