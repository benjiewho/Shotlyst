 ...
 */
function mergeOverlapping(intervals, durationSeconds, config) {
    ...
    for (const m of merged) {
        ...
        const mergedScore = (m.scores.motion + m.scores.audio + m.scores.scene + m.scores.speech) / 4;
        result.push({
            startTime: start,
            endTime: end,
            duration,
...
export function mergeCandidateMoments(signals, durationSeconds, config) {
    const intervals = [];
